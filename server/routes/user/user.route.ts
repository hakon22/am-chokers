...
export class UserRoute extends BaseRouter {
  ...
  public set = (router: Router) => {
    ...
    router.post(this.routes.user.updateTokens, passport.authenticate('jwt-refresh', { session: false }), this.userService.updateTokens);
    ...
    router.delete(this.routes.user.removeFavorites(), this.middlewareService.jwtToken, this.userService.removeFavorites);
    ...
    router.get(this.routes.user.changeLang, this.middlewareService.jwtToken, this.userService.changeLang);
  };
}
