      ...
      ? (
        <div className="d-flex gap-3 mt-3">
          {grade?.comment?.images.map(({ id: imageId, src, name: imageName }) => (
            <div key={imageId}>
              <Image src={src} width={50} height={50} alt={imageName} style={{ borderRadius: '7px' }} onClick={() => urlToBase64(src, setPreviewImage, setPreviewOpen, getBase64, setIsSubmit)} className="cursor-pointer" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" />
            </div>
          ))}
        </div>
      )
