  ...
  return orders.length
    ? (
      <div className="d-flex flex-column gap-4" style={{ width: '90%' }}>
        {orders.map((order) => (
          <Badge.Ribbon key={order.id} text={t(`statuses.${order.status}`)} color={getOrderStatusColor(order)}>
                ...
                <div className="d-flex flex-column col-8 gap-2">
                  {order.positions.map((position) =>
                    <Tooltip key={position.id} title={position.item.name} className="d-flex align-items-center gap-3" placement="left" color="#4d689e">
                      <span className="col-2 font-oswald lh-1 me-2">{truncateText(position.item.name)}</span>
